...
Open Historia drives every generative feature — the strategy advisor, leader diplomacy, timeline simulation, catalysts, stat sheets, and the game‑master console — through a single browser‑side AI layer under `src/Game/AI/`. The player's own API key talks **directly** to their chosen provider from the browser; there is no Open Historia backend in the loop for providers that allow browser calls. Every call tries direct first. When a provider blocks browser CORS (NVIDIA NIM, most OpenAI‑compatible gateways), the call falls back to the same‑origin `/api/ai/relay` — a Vercel serverless function on the hosted site, or a relay the player runs on a local install. For those CORS‑blocked providers the key transits the relay on its way to the provider; providers that allow browser calls (Gemini, OpenAI, Anthropic) never touch it. Two entry points sit on top of the transport: `callAI` for free‑form chat, and `runJsonTask` for schema‑validated structured "tasks" that mutate world state.
...
| `openai` | OpenAI | Native APIs | `callOpenAI` → `callOpenAIStyleChatCompletions` (`main.jsx:702`) | `https://api.openai.com/v1` | `providerFetch` (direct, relay fallback) | yes |
...
The whole security model is in the comment block at `main.jsx:240`. AI calls go **straight from the browser to the provider first** so the player's key only ever reaches the provider — never an Open Historia server or a community node — for providers that allow browser calls. When a provider blocks CORS, the call falls back to the same‑origin relay.

- **`PAGE_IS_LOCAL`** (`main.jsx:279`, from `isLocallyServed()`): true when the page is served from a machine the player controls — `localhost`/`127.0.0.1`/`::1`/`*.local` or the LAN private ranges `10.*`, `192.168.*`, `172.16–31.*`. It gates only the local‑endpoint guidance branch (below); the relay fallback itself fires on both hosted and local pages.
- **`providerFetch(url, options)`** (`main.jsx:352`): tries `directFetch`; on a CORS/network `TypeError` (not an abort) it remembers the origin in `relayOnlyOrigins` and retries through the same‑origin `/api/ai/relay` (`relayFetch`, `main.jsx:313`). A remembered origin skips the doomed direct attempt on later calls. `tryRelayFetch` (`main.jsx:328`) detects an absent relay (a static host answering the POST with HTML, or nothing answering) via the `x-open-historia-relay: 1` marker header and returns null so the original error surfaces.
- On the **hosted site** the relay is a Vercel serverless function (`api/ai/relay.js`): https‑only, GET/POST only, a header allowlist, a 4 MB body cap, no redirects, a 55 s timeout, and a DNS‑resolution SSRF guard. For CORS‑blocked providers the key transits this function on its way to the provider — that is the stated trade‑off. Providers that allow browser calls never reach it.
- If the hosted page tries to reach a **local** backend (Ollama/LM Studio) and the browser rejects it, `providerFetch` throws an actionable error telling the user to set `OLLAMA_ORIGINS`/enable CORS (`main.jsx:369`) — a local endpoint is never relayed from a hosted page because the cloud function cannot reach the player's machine.
- **Who uses the relay path**: only the `providerFetch` callers — `openai`, `openai-compatible`, `anthropic-compatible`, `nvidia`, `nvidia-nim-compatible`, and model discovery (`GET /models`). **Native Gemini and native Anthropic bypass `providerFetch` entirely** (plain `fetch`), because both explicitly allow browser calls (Anthropic via the `anthropic-dangerous-direct-browser-access: true` header). They are therefore always direct, with or without a relay.

`isLocalEndpoint(url)` (`main.jsx:298`) is the per‑endpoint sibling of `PAGE_IS_LOCAL`; it also gates local streaming (below).
