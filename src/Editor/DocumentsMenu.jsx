      ...
      {open && (
        <div style={{ ...panelSurface, marginTop: 6, width: 260, padding: 6, display: "flex", flexDirection: "column", gap: 2 }}>
          <div style={{ padding: "4px 8px 6px" }}>
            <div style={{ fontSize: 10.5, color: colors.mute, textTransform: "uppercase", letterSpacing: "0.03em", marginBottom: 4 }}>
              Made by
            </div>
            <input
              value={author || ""}
              ...
              style={{ width: "100%", boxSizing: "border-box", padding: "6px 8px", borderRadius: rounded.sm, border: `1px solid ${colors.hairline}`, background: colors.canvas, color: colors.ink, fontSize: 12.5, outline: "none" }}
            />
          </div>
          <div style={{ height: 1, background: colors.hairline, margin: "2px 0 4px" }} />
          ...
          <div style={{ height: 1, background: colors.hairline, margin: "4px 0" }} />
          ...
          <div style={{ height: 1, background: colors.hairline, margin: "4px 0" }} />
          <div style={{ padding: "2px 10px", fontSize: 10.5, color: colors.mute, textTransform: "uppercase", letterSpacing: "0.03em" }}>
            Saved maps
          </div>
          <div style={{ maxHeight: 240, overflowY: "auto" }}>
            {docs.length === 0 && (
              <div style={{ padding: "6px 10px", fontSize: 12, color: colors.mute }}>No saved maps yet</div>
            )}
            {docs.map((doc) => (
              <div key={doc.id} style={{ display: "flex", alignItems: "center", gap: 6, padding: "2px 4px" }}>
                <button
                  style={{ ...menuItem, padding: "6px 8px", flex: 1, background: doc.id === currentId ? colors.canvas : "transparent" }}
                  onClick={() => { onOpen(doc.id); close(); }}
                >
                  <span style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600 }}>{doc.name}</div>
                    <div style={{ fontSize: 10, color: colors.mute }}>{doc.regionCount} regions · {doc.featureCount} features</div>
                  </span>
                </button>
                <button
                  title="Delete"
                  onClick={async () => { await deleteDocument(doc.id); refresh(); }}
                  // Destructive red is a functional/semantic color, not brand chrome.
